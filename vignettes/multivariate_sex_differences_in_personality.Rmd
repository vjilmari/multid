...
This vignette uses openly available data from the "Big Five Personality Test,"  which is constructed with items from the International Personality Item Pool.  The dataset can be downloaded directly from <https://openpsychometrics.org> (by approach presented here <https://stackoverflow.com/a/66351588/8995170>). 
However, since CRAN checks do not allow internet resources to be required,  this vignette will use the full dataset if it can be retrieved, and otherwise  fall back to a smaller built-in subset (`example_big5`) that comes with the package.
...
if (ok) {
  ...
  dat <- rio::import(file.path(td, "BIG5/data.csv"))
  unlink(td)
  message("Using full BIG5 dataset from ", url)
} else {
  dat <- example_big5
  message("Could not retrieve external dataset. Using built-in fallback dataset `example_big5` (200 US participants).")
}
